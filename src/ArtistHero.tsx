    ...
    return (
        <>
            <style>{`
                ...
                @keyframes imgReveal { from{opacity:0;transform:scale(1.06)} to{opacity:1;transform:scale(1.02)} }
                @keyframes stripeIn  { from{transform:scaleY(0)} to{transform:scaleY(1)} }
                @keyframes nameIn    { from{opacity:0;transform:translateY(32px)} to{opacity:1;transform:translateY(0)} }
                @keyframes accentIn  { from{transform:scaleX(0);opacity:0} to{transform:scaleX(1);opacity:1} }
            ...
            >
                ...
                {/* Left vertical gold stripe (Hidden on small mobile for cleaner layout) */}
                <div
                ...
                <div className="relative z-10 w-full pb-10 sm:pb-14 px-6 sm:px-12 md:px-14 flex flex-col justify-end gap-6">
                    ...
                    {/* Bottom layout split block */}
                    <div
                        className="grid grid-cols-1 md:grid-cols-12 items-end gap-6 md:gap-12"
                        style={{ animation: "fadeUp 0.9s 0.85s both" }}
                    >
                        {/* Narrative Statement Copy */}
                        <p
                            className="font-outfit font-light leading-relaxed text-white/60 max-w-[460px] tracking-[0.01em] m-0 md:col-span-7 lg:col-span-8 text-sm md:text-xs lg:text-sm"
                        >
                    ...
                    {/* Infinite Scroll Line Indicator */}
                    <div
